...
a brief history:
SI units originated as a result that the centimetre - second - gram system, a system which seeked to define all units using only those three base units, however it had two sets of units for the electrostatis and elecromechanics, which was only resolved by introducing the Ampere to the system (which at this point was now metres - second - kilogram). These differed by the speed of light, then following maxwell's paper, which ultimately unified the two fields, did this relation become explained - with the proposed electromagnetic wave - aka light - which travels with constant speed, c = 1/sqrt(mu_0 epsilon_r).
