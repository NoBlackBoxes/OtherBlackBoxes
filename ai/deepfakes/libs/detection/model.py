...
class L2Norm(torch.nn.Module):
    def __init__(self, n_channels, scale=1.0):
        ...
        self.scale = scale
        self.eps = 1e-10
        self.weight = torch.nn.Parameter(torch.empty(self.n_channels).fill_(self.scale))
...
class model(torch.nn.Module):
    ...
    def forward(self, x):
        h = torch.nn.functional.relu(self.conv1_1(x), inplace=True)
        h = torch.nn.functional.relu(self.conv1_2(h), inplace=True)
        h = torch.nn.functional.max_pool2d(h, 2, 2)

        h = torch.nn.functional.relu(self.conv2_1(h), inplace=True)
        h = torch.nn.functional.relu(self.conv2_2(h), inplace=True)
        h = torch.nn.functional.max_pool2d(h, 2, 2)

        h = torch.nn.functional.relu(self.conv3_1(h), inplace=True)
        h = torch.nn.functional.relu(self.conv3_2(h), inplace=True)
        h = torch.nn.functional.relu(self.conv3_3(h), inplace=True)
        f3_3 = h
        h = torch.nn.functional.max_pool2d(h, 2, 2)

        h = torch.nn.functional.relu(self.conv4_1(h), inplace=True)
        h = torch.nn.functional.relu(self.conv4_2(h), inplace=True)
        h = torch.nn.functional.relu(self.conv4_3(h), inplace=True)
        f4_3 = h
        h = torch.nn.functional.max_pool2d(h, 2, 2)

        h = torch.nn.functional.relu(self.conv5_1(h), inplace=True)
        h = torch.nn.functional.relu(self.conv5_2(h), inplace=True)
        h = torch.nn.functional.relu(self.conv5_3(h), inplace=True)
        f5_3 = h
        h = torch.nn.functional.max_pool2d(h, 2, 2)

        h = torch.nn.functional.relu(self.fc6(h), inplace=True)
        h = torch.nn.functional.relu(self.fc7(h), inplace=True)
        ffc7 = h
        h = torch.nn.functional.relu(self.conv6_1(h), inplace=True)
        h = torch.nn.functional.relu(self.conv6_2(h), inplace=True)
        f6_2 = h
        h = torch.nn.functional.relu(self.conv7_1(h), inplace=True)
        h = torch.nn.functional.relu(self.conv7_2(h), inplace=True)
        f7_2 = h
